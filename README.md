...
3. 세종대 지도 안내판 기물을 화면 중앙에 맞춥니다.
4. 안내판이 인식되면 카메라 화면 위에 3D 기린과 게임식 대사창이 나타납니다.
5. 기린을 탭하면 퀘스트형 미션으로 이동합니다.
6. 3자리 코드 `428`을 입력하면 미션 성공 화면이 나옵니다.
7. `오늘 23:59까지` 사용할 수 있는 캠퍼스 쿠폰이 발급됩니다.

## 인식 대상 기물

현재 데모는 세종대 지도 안내판 기물을 인식 대상으로 사용합니다. 참고 이미지는 다음 경로에 있습니다.

- `public/sejong-map-fixture.jpeg`

실제 시연에서는 해당 안내판 앞에서 스마트폰 카메라를 켜고, 밝은 지도 영역이 화면 중앙 프레임 안에 들어오도록 비추면 됩니다. 데모 앱 첫 화면의 `인식할 기물 예시 보기` 링크로 참고 이미지를 확인할 수 있습니다.

현재 구현은 시연 안정성을 위해 안내판의 밝은 지도 패널, 청록색 지도 영역, 분홍 번호점 조합을 카메라 프레임 중앙에서 인식합니다. GPS 기반 AR은 사용하지 않습니다.
...
Pages 버전도 같은 사용자 흐름을 제공합니다. 스마트폰으로 접속한 뒤 `AR 스캔 시작`을 누르고 세종대 지도 안내판 기물을 비추면 됩니다.
...
- 이미지 마커 감지: 중앙 프레임의 세종대 지도 안내판 색상/패턴 인식
