...
`Map` 탭은 Kakao Maps JavaScript SDK를 불러올 수 있도록 준비되어 있습니다. 실제 카카오 지도를 보려면 Kakao Developers에서 JavaScript 키를 만들고, GitHub Pages 주소 뒤에 다음처럼 붙여서 접속합니다.

```text
https://campusdrop.github.io/campusdrop_v2_prototype/?kakaoKey=YOUR_JAVASCRIPT_KEY
```

한 번 입력한 키는 브라우저 로컬 저장소에 저장되어 같은 기기에서는 다시 붙이지 않아도 됩니다. 키가 없을 때는 지도 자리의 구성과 크루/보물/매장 오버레이를 확인할 수 있는 프리뷰가 표시됩니다.
