...
4. 마커가 인식되면 카메라 화면 위에 회전하는 3D NPC가 나타납니다.
5. NPC를 탭하면 퀘스트형 미션으로 이동합니다.
...
- AR 오브젝트: `sejongGF.glb`를 사용한 회전/발광 3D NPC
- 앱 흐름: React 상태 전환
...
5. `anchor.group`에 `sejongGF.glb` 모델을 붙이면 이미지 위에 NPC 오브젝트가 고정됩니다.
