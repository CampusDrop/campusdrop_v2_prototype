...
`Map` 탭은 Mapbox GL JS를 불러와 `mapbox://styles/mapbox/dark-v11` 다크 테마 지도를 표시합니다. Mapbox access token에는 아래 도메인을 허용해 둡니다.
...
Mapbox access token은 저장소에 커밋하지 않고, GitHub Pages 주소 뒤에 다음처럼 붙여 접속합니다.

```text
https://campusdrop.github.io/campusdrop_v2_prototype/?mapboxToken=YOUR_MAPBOX_ACCESS_TOKEN
```
